import { CheckCircle2, CircleDot, Menu, X } from "lucide-react";
...
export function App() {
  ...
  return <main className={open ? "overlay-shell open" : "overlay-shell collapsed"}>
    {!open ? <button className="open-launcher" onClick={() => setOpen(true)} aria-label="Open Bannerlord Twitch"><Menu /><span>BLT</span></button> : null}
    {open ? <div className="overlay-window">
      <header className="top-bar"><h1>Bannerlord Twitch</h1><span className={state.connected ? "connection connected" : "connection disconnected"}><i />{state.connected ? "Connected" : "Game offline"}</span><button className="close-button" onClick={() => setOpen(false)} aria-label="Collapse overlay"><X /></button></header>
