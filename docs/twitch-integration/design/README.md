...
`blt-logo-v2.png` applies the same flat cartoon treatment to the BLT shield-and-sword monogram. The realistic `v1` logo remains as a rollback reference.

The production UI uses eight independently generated transparent files under `assets/command-icons/`: `hero.png`, `battle.png`, `kingdom.png`, `equipment.png`, `progression.png`, `tournament.png`, `community.png`, and `general.png`. It no longer contains or crops from a shared sprite sheet. Each `CommandIcon` renders a normal `<img>` element, allowing real padding and independent replacement of every category asset.
