...
## Verification recorded during implementation (2026-09-05)

- Main and integration game projects: Release builds against installed Bannerlord assemblies.
- Both branches: engine-independent BLT console suites passed, including prestige policy tests.
  The local machine has .NET 9, so these were built with `TargetFramework=net9.0`;
  the committed test target remains .NET 8. Game assemblies retain .NET Framework 4.8.
- Integration backend: 10 tests passed, including old/new viewer snapshot compatibility.
- Frontend: 17 tests passed, including preview acknowledgement, expiry, stale eligibility,
  capped perks, missing snapshots and offline handling. Production build passed.
- Browser: `npx playwright test --config playwright.prestige.config.ts` passed using installed
  Chrome and a mocked private game connection, with desktop/mobile screenshots and no page errors.
  Browser plugin was unavailable; the existing Playwright tooling was used. External font/helper
  requests are stubbed for this offline test. Screenshots are in frontend `test-results`.
- Live campaign reset, save/load, death/inheritance and forced engine rollback: **not run**.
  These require a running disposable Bannerlord campaign before release. No deployment performed.
